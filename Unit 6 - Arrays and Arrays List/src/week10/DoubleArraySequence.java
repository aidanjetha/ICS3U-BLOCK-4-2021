...
public class DoubleArraySequence {
    ...
    public void addAfter(double d) {
        /**
         * ensureCapacity(manyItems + 1); for (int i = manyItems; i > currentIndex + 1;
         * i--) { data[i] = data[i - 1]; } data[currentIndex + 1] = currentIndex;
         * manyItems++; currentIndex++; //advance();
         */
        ...
        //Ensure capacity working when not needed 
    ...
    public void ensureCapacity(int minimumCapacity) {
         /**
         * double biggerArray[]; if (data.length < minimumCapacity) { biggerArray = new
         * double[minimumCapacity]; System.arraycopy(data, 0, biggerArray, 0,
         * manyItems); data = biggerArray; }
         */

        
        int ensuredCapacity = data.length;
        /*
        if(ensuredCapacity*2<minimumCapacity){
            double[] arr = new double[minimumCapacity;
            System.arraycopy(data, 0, arr, 0, manyItems);
            data = arr;
        }else{
            double[] arr = new double[ensuredCapacity*2];
            System.arraycopy(data, 0, arr, 0, manyItems);
            data = arr;
        }
        */
        if(ensuredCapacity < minimumCapacity){
            //ensuredCapacity *= 2;
            ensuredCapacity = minimumCapacity;
        }
        
        double[] arr = new double[ensuredCapacity];
        System.arraycopy(data, 0, arr, 0, manyItems);
        data = arr;
    }
